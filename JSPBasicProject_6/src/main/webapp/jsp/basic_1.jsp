...
<%--
		...
		= response : 브라우저로 전송할 응답 정보 / 화면 이동
		= pageContext : 웹 흐름 제어
						include / forward
		...
		----------------------- 기본
		 HTTP의 기본 stateless : 무상태 (데이터)
		 	...
		 	-------------------------------------------------
		 	저장위치      클라이언트				서버
		 	저장 위치     문자열					서버메모리 (객체단위)
		 	보안성	    낮다					높다
		 	지속시간	    브라우저 종료후에도 유지	브라우저 종료 / 로그아웃 => 소멸
		 				설정이 가능 
		 				setMaxAge(1800)
		 	...
		 	1. HTTP 기본 (요청 == 응답 => 연결 종료)
		 		| 필요한 데이터 저장할 수 있는 기억 장치가 필요
		 		| cookie / session => 면접 단골 질문
		 			|		  | 접속한 서버에 저장
		 		  본인 컴퓨터에 저장
...
<body>
  출력버퍼크기:<%=out.getBufferSize() %><br>
  남아 있는 버퍼:<%=out.getRemaining() %><br>
  현재 사용중인 버퍼: 
  		<%
  			int size=out.getBufferSize()-out.getRemaning();
				out.println(size);
  		%>
</body>
</html>