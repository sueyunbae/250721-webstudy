...
<%--
	JSP ----  서버 ---- JSP
	 ...
	 --------------------
	 	...
	 	Ajax 동작 순서
	 	-----------
	 	Ajax : 페이지 전체를 새로고침을 하지 않고 서버에서 필요한 데이터만 주고 받는 기술
	 			=> Vue, React, next, nust...
	 			=> jquery : 4,5
	 	  | ==> axios
...
// 요청 = 응답 데이터 받기
function sendRequest(url,params,callack,method)
{
	/*
		$.ajax({
			type => method
	...
	*/
	...
	if(httpMethdo!='GET' && httpMethod!='POST')
	{
		httpMethod='GET'
	}
	// 3 params => ?id=admin
	...
	// 응답 => responseXML / responseXML(JSON)
	// JSP => 70%, 30% => ajax
}
function send()
{
